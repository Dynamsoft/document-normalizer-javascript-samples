<template>
  <div style="width: 100%; height: 100%">
    <div class="video-normalizer-btns">
      <button class="editor-border-btn" @click="confirmQuadForNormalization">confirmQuadForNormalization</button>
      <button class="end-editor-btn" @click="normalizeWithConfirmedQuad">normalizeWithConfirmedQuad</button>
    </div>
...
export default {
  setup() {
    ...
    const elVnr = ref(null);
    let pNormalizer = null;
    let pCameraEnhancer = null;
    onMounted(async () => {
      try {
        pCameraEnhancer = pCameraEnhancer || await CameraEnhancer.createInstance();
        pNormalizer = pNormalizer || await DocumentNormalizer.createInstance();
        await pCameraEnhancer.setUIElement(elRefs.value);
        await pNormalizer.setImageSource(pCameraEnhancer, {resultsHighlightBaseShapes: DrawingItem});
        await pNormalizer.startScanning(true);
    ...
    const confirmQuadForNormalization = () => {
      pNormalizer.confirmQuadForNormalization();
    }
